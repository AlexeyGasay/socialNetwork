...
class FriendsController {
  ...
  async searchPeople(req, res) {
    ...
    try {
      ...
      const foundPeople = await db.query(`
        SELECT username, user_id FROM user_list
        WHERE username ILIKE '%${username}%'
        ORDER BY user_id DESC
        LIMIT 5
        `);

      res.json(foundPeople.rows);
  ...
  async getUserInfo(req, res) {
    try {
      ...
      const userInfo = await db.query(`SELECT user_id, username, user_avatar FROM user_list WHERE user_id = ${user_id}`);
